...
6. **스크립트는 보호하고 그것을 부르라는 *절차*를 놓쳤다.** 시크릿 게이트의 `scan.sh`·`scan_test.sh`·allowlist는 등재했는데, **언제·어떻게 그걸 부를지 지시하는 `SKILL.md`**는 빠졌다. loop가 스크립트를 한 줄도 안 건드리고 절차만 약화시키면 게이트가 꺼진다. **여섯 번째 발생.**

**일반화**: 규칙 · 그 규칙이 지배하는 **명령** · 그 규칙을 지키는 **검사** · 그 규칙이 적용되는 **자기 자신** · 그 규칙이 지배하는 **호출자** · 그 규칙을 **부르라고 지시하는 절차**, 여섯이 같은 변경에서 함께 움직여야 한다. 그리고 **검사는 작성이 아니라 실행·확인까지가 검사다.**

**더 중요한 메타 교훈 — 열거를 멈추고 완결성으로 바꿔라.** 이 PR에서만 같은 클래스를 여섯 번 놓쳤고, 매번 "이번엔 다 찾았다"고 생각했다. 손으로 열거하는 한 다음 항목도 놓친다. 그래서 마지막 수정은 항목 추가가 아니라 **완결성 체크 확장**이었다 — `.claude/agents/*.md`와 레포-로컬 게이팅 스킬의 `*.md`를 glob으로 순회해 **미등재가 있으면 실패**하고, **매치 0건이면 vacuous pass를 막으려 fail-closed**한다. 앞으로 추가되는 지시 표면은 사람이 기억하지 않아도 잡힌다. #64가 ADR에 대해 한 일을 지시 표면에 대해서도 한 셈이다.
