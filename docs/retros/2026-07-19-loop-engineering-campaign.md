...
5. **존재하지 않는 파일을 보호하려 했다.** 4번을 고치며 `codex-pr-review` 스킬도 등재했는데, 그건 **사용자 전역 `~/.claude/skills/`에 살아 이 레포 CODEOWNERS로 보호 불가**하다. 내가 새로 쓴 완결성 테스트가 "매치 0건 → vacuous pass 방지"로 **fail-closed하며 잡았다** — 처음으로 *내가 만든* 검사가 *내 실수*를 잡은 순간이다. 신뢰 경계 밖이라는 한계를 book했다.
