...
**이 회고 PR 자신이 그 실패 클래스를 세 겹으로 재생산했다** — 세션에서 가장 값진 데이터 포인트다.

1. **규칙과 명령이 따로 놀았다.** "codex는 `--base origin/main`"을 운영 불변식에 적어놓고 **같은 파일 말미의 실제 호출 명령은 고치지 않았다.** 적대 리뷰: *"에이전트는 흐름 끝의 구체적 명령을 실행하지, 앞의 산문 불변식과 대조하지 않는다."*
2. **그 수정에서 쓴 회귀 검사가 자기 문서를 잡아 영구 red였다.** 금지 리터럴을 검사 설명에 그대로 써서, 검사가 항상 실패하는 형태였다. 더 나쁜 건 **내가 그 검사를 실행해 `❌ 회귀 존재`를 출력하고도 백그라운드 출력이라 읽지 않고 push**한 것이다 — 자기 검증을 자기가 안 읽었다.
3. **새 규칙이 자기 자신에게 적용되지 않았다.** "게이트로 승격하면 sacred 등재"를 CLAUDE.md에 신설하면서, 이 PR이 바로 그 역할로 승격시킨 **CLAUDE.md 자신**(규칙의 상시-로드 소스)과 **워커 정의**(codex 호출·ADR 충돌 처리의 실행 절차)를 등재하지 않았다. 게이트 코드를 한 줄도 안 건드리고 게이트를 끄는 경로가 열려 있었다. 같은 클래스의 **네 번째** 발생이자, 규칙 자신이 범인인 첫 사례.

세 건 모두 외부 적대 리뷰가 잡았다. 수정은 각각 명령 교체 / 금지 리터럴을 쓰지 않는 검사 형태 + **실제 실행해 green 확인** / CLAUDE.md·`.claude/agents/` sacred 등재 + fail-closed 테스트로 했고, 사고 경위를 그 자리에 각주로 고정했다.

4. **leaf는 보호하고 orchestrator를 놓쳤다.** 워커 정의(`go-tdd-implementer.md`)를 sacred 등재하면서, 그것을 *호출하는* `dispatch-issue/SKILL.md`(이슈 적격성·risk:critical 정지 조건·계정 고정·워커 위임 강제·PR 핸드오프 결정)는 빼놨다. 워커만 보호해도 오케스트레이터를 고치면 게이트는 그대로 꺼진다. **다섯 번째 발생.**
5. **존재하지 않는 파일을 보호하려 했다.** 4번을 고치며 `codex-pr-review` 스킬도 등재했는데, 그건 **이 레포가 아니라 사용자 전역 스킬 디렉터리에 설치돼 있어 레포 CODEOWNERS로 보호 불가**하다. 내가 새로 쓴 완결성 테스트가 "매치 0건 → vacuous pass 방지"로 **fail-closed하며 잡았다** — 처음으로 *내가 만든* 검사가 *내 실수*를 잡은 순간이다. 신뢰 경계 밖이라는 한계를 book했다.

**일반화**: 규칙 · 그 규칙이 지배하는 **명령** · 그 규칙을 지키는 **검사** · 그 규칙이 적용되는 **자기 자신** · 그 규칙이 지배하는 **호출자**, 다섯이 같은 변경에서 함께 움직여야 한다. 그리고 **검사는 작성이 아니라 실행·확인까지가 검사다.**

**희망적인 신호**: 이 마지막 사이클에서 #64가 세운 enforcement 자기보호 완결성 체크가 **내가 새로 추가한 테스트 파일의 미등재를 자동으로 잡았고**, 내가 새로 쓴 instruction-surface 완결성 체크가 **내 잘못된 경로를 fail-closed로 잡았다**. self-catch 0이던 세션이 끝날 무렵, 이번 세션이 만든 기계가 처음으로 자기 실수를 잡기 시작했다 — 그것이 이 캠페인의 실질 산출물이다.
