...
**이 회고 PR 자신이 그 실패 클래스를 재생산했다.** 개선안으로 "codex는 `--base origin/main`"을 운영 불변식에 적어놓고, **같은 파일 말미의 실제 호출 명령은 `--base main`으로 남겨뒀다.** 적대 리뷰가 잡았다(PR #81 [high]): *"에이전트는 흐름 끝의 구체적 명령을 실행하지, 앞의 산문 불변식과 대조하지 않는다."* — 규칙을 문서화하는 행위 자체가 그 규칙의 twin(규칙이 지배하는 **명령**)을 놓칠 수 있다는 증거다. 이 세션에서 **가장 값진 데이터 포인트**이며, 그래서 수정은 명령 교체에 그치지 않고 **회귀 검사 방법(`grep -n -- "--base main"`이 무반환이어야 함)과 이 사고 경위를 그 자리에 각주로 고정**했다.
