...
- **(c) unclean 종료 후에는 non-reconstructable 전역 halt를 보수적으로 halted 부팅한다 — clean-shutdown sentinel.** durable store가 write 불가한 구간에 발생한 non-reconstructable 전역 halt(토큰 갱신 실패)는 durable하게 보존할 물리적 수단이 없다(store가 곧 그 수단). 재구성 가능 신호(ambiguous·order-failure)는 (b)의 재계산·re-count로 복구되지만, non-reconstructable 트리거는 그 durable write가 실패한 뒤 크래시하면 재시작이 halt 사실을 알 수 없다 — 그리고 **전역 halt는 사유가 사라져도 자동 재개하지 않고 사람 수동 clear를 요구한다(ADR-0004 point 6).** 그러므로 "증상이 멈췄으니 안전"으로 자동 재개하는 것은 point 6 위반이다. 이를 닫기 위해 **정상 graceful shutdown 시에만 durable clean-shutdown 마커를 기록**하고, 재시작 시 그 마커의 유무로 갈린다: 마커가 있으면 durable 상태를 신뢰(정상 recovery), **마커가 없으면(크래시·store-error·kill = unclean 종료) non-reconstructable 전역 halt를 보수적으로 halted 부팅해 operator 수동 clear까지 fail-closed로 유지**한다(ADR-0007의 initial-authorization 게이트와 같은 계열의 부팅 halt). unclean 종료 후 사람 확인을 요구하는 비용을 지불하지만 — store-write 실패·비정상 종료는 드문 사건이고 — fail-open보다 fail-closed가 point 6 계약이다.
...
- (후속) clean-shutdown 마커 스키마(store), graceful shutdown 시 마커 기록 배선(cmd/bot — 구현 이슈 #36), unclean 부팅 감지 및 보수적 halted 전환은 구현 이슈에서 크래시-타이밍 테스트로 커버한다("정상 종료 후 recovery" vs "unclean 종료 후 non-reconstructable halt 보수적 halted"를 반드시 포함).
