...
- **(c) unclean 종료 후에는 non-reconstructable 전역 halt를 보수적으로 halted 부팅한다 — clean-shutdown sentinel.** durable store가 write 불가한 구간에 발생한 non-reconstructable 전역 halt(토큰 갱신 실패)는 durable하게 보존할 물리적 수단이 없다(store가 곧 그 수단). 재구성 가능 신호(ambiguous·order-failure)는 (b)의 재계산·re-count로 복구되지만, non-reconstructable 트리거는 그 durable write가 실패한 뒤 크래시하면 재시작이 halt 사실을 알 수 없다 — 그리고 **전역 halt는 사유가 사라져도 자동 재개하지 않고 사람 수동 clear를 요구한다(ADR-0004 point 6).** 그러므로 "증상이 멈췄으니 안전"으로 자동 재개하는 것은 point 6 위반이다. 이를 닫기 위해 clean-shutdown 마커를 **2-phase lifecycle + eligibility 규칙**으로 관리한다(sentinel 자신이 fail-open을 만들지 않도록):
  - **(부팅)** 제출을 열기 전에 durable "running"(dirty) 마커를 기록해 **이전 run이 남긴 clean 마커를 먼저 무효화**한다 — 이 부팅 write가 실패하면(store 이상) halted/fail-closed로 부팅한다. 이 단계가 없으면 이전 정상 종료의 stale clean 마커가 다음 run의 크래시·halt 유실을 마스킹한다(sentinel fail-open #1).
  ...
  - **(재시작)** clean 마커가 있으면(정상 종료 + 미해결 halt 없음) durable 상태를 신뢰(정상 recovery)하고, **없으면(running 마커만 = 크래시·store-error·kill·pending-halt-미영속 = unclean 종료) 재구성 가능 신호는 (b)의 재계산·re-count로 복구하되 non-reconstructable 전역 halt는 보수적으로 halted 부팅해 operator 수동 clear까지 fail-closed로 유지**한다(ADR-0007의 initial-authorization 게이트와 같은 계열의 부팅 halt).

  unclean 종료 후 사람 확인을 요구하는 비용을 지불하지만 — store-write 실패·비정상 종료는 드문 사건이고 — fail-open보다 fail-closed가 point 6 계약이다. 2-phase(부팅 running-마킹 + 조건부 clean)가 stale clean 마커와 pending-halt를 잃은 run의 clean 인증이라는 sentinel 자체의 두 fail-open을 닫는다.
