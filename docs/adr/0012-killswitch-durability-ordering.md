...
- **(c) unclean 종료 후에는 non-reconstructable 전역 halt를 보수적으로 halted 부팅한다 — clean-shutdown sentinel.** durable store가 write 불가한 구간에 발생한 non-reconstructable 전역 halt(토큰 갱신 실패)는 durable하게 보존할 물리적 수단이 없다(store가 곧 그 수단). 재구성 가능 신호(ambiguous·order-failure)는 (b)의 재계산·re-count로 복구되지만, non-reconstructable 트리거는 그 durable write가 실패한 뒤 크래시하면 재시작이 halt 사실을 알 수 없다 — 그리고 **전역 halt는 사유가 사라져도 자동 재개하지 않고 사람 수동 clear를 요구한다(ADR-0004 point 6).** 그러므로 "증상이 멈췄으니 안전"으로 자동 재개하는 것은 point 6 위반이다. 이를 닫기 위해 clean-shutdown 마커를 **2-phase lifecycle + eligibility 규칙**으로 관리한다(sentinel 자신이 fail-open을 만들지 않도록):
  - **(표현)** sentinel은 clean·running을 별도 레코드로 두지 않고 **단일 durable lifecycle 상태**(값 `clean` | `running` 하나의 필드/행, 또는 monotonic boot-epoch)로 표현해 두 값이 공존할 수 없게 한다 — 별도 레코드면 크래시가 둘을 함께 남겨 recovery가 stale `clean`을 신뢰할 수 있다(sentinel fail-open #1의 잔재).
  - **(부팅)** 제출을 열기 전에 이 상태를 **atomic하게 `running`으로 전환**(이전 `clean`을 같은 write로 덮어씀)한다 — 이 부팅 write가 실패하면(store 이상) halted/fail-closed로 부팅한다. 이 단계가 없으면 이전 정상 종료의 stale `clean` 상태가 다음 run의 크래시·halt 유실을 마스킹한다(sentinel fail-open #1).
  - **(종료)** 정상 graceful shutdown이 **(i) 미해결 non-durable 안전 상태(durable write가 실패한 pending 전역 halt)가 없고 (ii) 정상 종료 경로일 때에만** clean 마커를 기록한다. 미해결 pending 전역 halt가 있으면 먼저 durable persist하고, persist가 **실패하면 clean 마커를 쓰지 않는다**(sentinel을 unclean로 남긴다) — 그러지 않으면 pending halt를 durable 기록 없이 잃은 run이 스스로를 clean으로 인증한다(sentinel fail-open #2).
  - **(재시작)** 단일 상태가 `clean`이면(정상 종료 + 미해결 halt 없음) durable 상태를 신뢰(정상 recovery)하고, **`running`이면(이전 clean 이력과 무관 = 크래시·store-error·kill·pending-halt-미영속 = unclean 종료) durable 전역 halt 상태가 `pending`/`halted`이거나 재구성 신호의 재계산이 halt를 내면 halted로, 그 밖에도 durable trip이 실패한 pending 전역 halt를 배제할 수 없으면 보수적으로 halted 부팅해 operator 수동 clear까지 fail-closed로 유지**한다(ADR-0007의 initial-authorization 게이트와 같은 계열의 부팅 halt). 단일 상태이므로 recovery는 "현재 값이 `running`인가"만 보고 판정한다 — 낡은 `clean`이 새 크래시 run을 인증하는 공존 경로가 없다.

    **중요 — unclean 부팅의 전역 halt 보존은 트리거 종류에 비대칭이 아니다.** durable `TripHalt`가 실패한 pending 전역 halt는 **재구성 가능 트리거(ambiguous 빈발)여도** 유실될 수 있다: `TripHalt` durable write가 실패해 in-memory pending인 동안, 조회는 계속 돌므로 reconciler가 그 `unresolved-ambiguous` intent를 resolve해 재계산 evidence를 지운 뒤 unclean 종료하면, 재시작 시 재계산할 intent도 없고 durable halt도 없다 → 유실(point 6 위반). 이를 닫기 위해 **(i) 전역 halt durable 상태를 `pending`(트립 개시) → `halted`(완료) 2단계로 두고**(store가 write 가능한 동안 `pending`도 durable하므로 unclean recovery가 `pending`을 halted로 취급), **(ii) reconciler는 durable `pending`(또는 `halted`) 전역 halt가 서 있는 동안 그 evidence(`unresolved-ambiguous` intent) resolution보다 전역 halt persistence를 우선한다(persistence-wins) — 즉 pending 전역 halt를 durable 확정하기 전에는 재계산 evidence를 지우지 않는다.** store 자체가 write 불가한 순간(`pending`도 못 씀)의 halt는 (a)의 제출 fail-closed와 clean-shutdown sentinel의 unclean 보수적 halted가 함께 덮는다.

   > **Amend (ADR-0014)**: 이 (ii) persistence-wins 계약이 상정한 행위 — "reconciler가 `unresolved-ambiguous` intent를 resolve해 재계산 evidence를 지운다" — 는 ADR-0014가 소스 실측+적대검증으로 확인한 결과 **구조적으로 도달 불가능**하다: ambiguous intent는 `MarkerAcked`(유일하게 orderId를 싣는 마커)가 없어 orderId 핸들이 없고, reconciler는 orderId 없이는 `GET /orders/{orderId}`로 결과를 조회할 수단이 없어 auto-resolve가 원천 불가이며, `finalizeFullyAudited`가 `ResolvedAt != nil`을 강제해 prune도 불가하다. 따라서 reconciler는 이 evidence를 **결코 지우지 않으므로** D1(c) hazard가 이 경로에서 unreachable하다(vacuous가 아니라 구조적). 그 결과 ADR-0014는 reconciler에 **명시적 halt-phase 가드를 두지 않는다**(no-guard) — order-failure evidence는 count-before-resolve(Decision 3)로 이미 봉쇄되고, 명시적 가드를 두면 사람-수동-clear 전용 전역 halt·bootHalt가 장기 지속되는 동안 confirmed 주문 resolve·감사 finalize를 동결시키는 availability 자해가 된다(`HasUnpersistedPendingHalt()`는 bootHalt에도 true라 loss-window 신호로도 부적합). **stateless 구현자는 이 (ii)를 근거로 reconciler에 halt-phase prune 가드를 재도입하지 말 것** — 상세·복원 조건(ambiguous에 resolvable 핸들을 주는 변경이 도입되면 loss-window 전용 가드 복원)은 ADR-0014 Decision 8·Alternatives.

  unclean 종료 후 사람 확인을 요구하는 비용을 지불하지만 — store-write 실패·비정상 종료는 드문 사건이고 — fail-open보다 fail-closed가 point 6 계약이다. 단일 lifecycle 상태 + 부팅 atomic `running` 전환 + 조건부 `clean`이 (stale-clean 공존·pending-halt를 잃은 run의 clean 인증) sentinel 자체의 fail-open을 닫는다.

이 세 겹이 없는 해석 — 예컨대 durable write 에러를 in-memory pending으로만 처리하거나, unclean 종료 후 clean-shutdown 마커 없이 non-reconstructable halt를 unhalted로 부팅하는 구현 — 은 point 7의 재시작 fail-open과 point 6의 자동-재개 금지를 위반하므로 금지다.
...
- **재시작 시 미해결 안전 신호가 있으면 무조건 fail-closed + 사람 검토** — 기각: crash-loop에서 재시작마다 사람 개입을 요구해 무인성을 과도하게 희생한다(ADR-0004가 같은 이유로 기각한 대안과 동형). count-first + reconciler re-count로 자동 복구가 가능하면 그쪽이 비례적이다. **단 재구성 불가 전역 halt 트리거의 durable write 실패 케이스에서는** 이 보수성(clean-shutdown sentinel + unclean 부팅 시 보수적 halted)이 유일한 fail-closed 수단이다(Decision point 1(c)) — 거기서는 재계산으로 복구할 증거 자체가 없기 때문이다.
- **durable-write 실패 창을 "위험 소멸 창"으로 booking(사람 clear 불요)** — 기각(codex 하드닝 지적): 전역 halt는 사유가 소멸해도 자동 clear되지 않고 사람 수동 clear가 계약이다(ADR-0004 point 6 — "시스템 이상은 근본 원인 미해결 채 재개하면 또 사고"). 토큰 갱신 실패로 트립된 전역 halt가 durable write 실패로 유실된 뒤, 토큰이 복구됐다고 자동 재개하는 것은 증상 소멸을 근본 원인 확인으로 오인하는 point 6 위반이다. clean-shutdown sentinel로 unclean 종료 후 보수적 halted 부팅해 operator clear를 강제한다.
...
- (계약 전파) reconciler(ADR-0003, 구현 이슈 #35)는 unresolved intent를 `rejected`로 확정할 때 killswitch에 order 실패를 report해야 한다(count-first 재시작 복구의 완성). ambiguous 빈도 재계산 계약(ADR-0004 point 7)과 같은 표면이다. **추가로 persistence-wins: durable 전역 halt가 `pending`/`halted`인 동안 reconciler는 그 재계산 evidence(`unresolved-ambiguous` intent)를 resolve하기 전에 전역 halt persistence를 우선한다 — durable `TripHalt` 실패로 in-memory pending인 전역 halt가 evidence 소멸로 유실되는 것을 막는다(Decision point 1(c)).**
- (비용) 전역 halt durable 상태가 `none`→`pending`(트립 개시)→`halted`(완료)→`none`(clear)의 2-단계 lifecycle을 가진다 — `pending`은 "트립이 durable하게 개시됐으나 완료 전"을 뜻해 unclean recovery가 이를 halted로 취급한다. store write가 가능한 동안에는 `pending`→`halted`가 즉시 이어지므로 정상 경로 비용은 미미하고, store-error 순간에만 `pending`이 못 써져 (a)의 제출 fail-closed로 넘어간다.
...
- (비용) **clean-shutdown sentinel**: 정상 graceful shutdown 시 durable clean-shutdown 마커를 기록하고, unclean 종료(크래시·store-error·kill) 후 재시작은 non-reconstructable 전역 halt를 보수적으로 halted 부팅해 operator 수동 clear를 요구한다(Decision point 1(c)). store-write 실패·비정상 종료가 드물어 정상 무인 운영의 비용은 제한적이나, crash-loop 상황에서는 사람 개입 빈도가 오른다 — fail-open보다 fail-closed를 택한 의도된 비용이다.
- (후속) clean-shutdown sentinel의 **2-phase lifecycle** — 부팅 시 running(dirty) 마커 기록으로 이전 clean 무효화 / graceful shutdown 시 조건부 clean 마커(미해결 non-durable halt 없을 때만) / unclean 부팅 시 보수적 halted — 의 스키마(store)와 배선(cmd/bot 부팅·graceful shutdown — 구현 이슈 #36)은 구현 이슈에서 크래시-타이밍 테스트로 커버한다. 필수 케이스: (i) 정상 종료 후 recovery, (ii) unclean 종료 후 non-reconstructable halt 보수적 halted, (iii) **stale clean 마커**(이전 정상 종료 마커가 남은 채 다음 run 크래시 → 부팅 running-마킹이 무효화하는지), (iv) **pending-halt 미영속 run의 clean 거부**(durable halt write 실패한 pending 상태로 graceful shutdown → clean 마커 안 써지는지).
