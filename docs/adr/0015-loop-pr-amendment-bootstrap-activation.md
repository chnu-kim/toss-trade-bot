...
5. **이 세션에서 에이전트가 admin으로 수행 가능한 최대 활성화 범위를 명시한다.** precondition ① 산출물(verdict-gate required 후보·presence-check 3-pillar) 검증 · ②의 실측 스크립트 **문법·prereq 리허설과 결과 해석·배선**(실행 자체는 아님 — point 4) · ④ protection-probe 절차 준비·문서화 · **⑤ verdict 레짐 실측 전체**(point 4 (g)에 따라 leg별 — codex leg·N-of-2면 Claude leg 각각). **⑤는 인젝션 1건이 아니라 다섯 항목이다(codex #73 R6 정정)**: (1) 인젝션 내성, (2) 같은 head SHA sticky 무재발행, (3) PR당 연속 비-approve 상한 N, (4) **여러 PR에 걸친 전역 상한 M**(새 PR 개설로 예산 리셋 안 됨·loop-위조 불가 소스에서 집계), (5) 전역 해제가 chnu-kim actor `workflow_dispatch`로만 가능 + **적격 가드**(fork·비-`mechanu[bot]` PR은 verdict check 미설정·머지 무동작). 인젝션만 통과하고 flip하면 재시도·재샘플링·부적격 PR 경로가 **비-sacred 유일 게이트**에서 미검증인 채 열린다. **그러나 이 범위의 어느 것도 main의 count를 내리지 않고, App key를 environment에 넣지 않으며, 사람계정 자격증명을 바꾸지 않는다** — 그 셋이 정확히 물리 경계 밖이다.
...
6. **남는 irreducible 사람-액션 핸드오프를 실제 실행 순서로 열거한다(적대 렌즈-3 Finding1 반영).** ADR-0011 라운드 7이 `(3)→(1)` 순서 + 프로비저닝-시점 rotate로 닫은 durable App-key 탈취 창은, 핸드오프를 dependency 역순(provision을 narrowing보다 먼저)으로 나열하면 stateless 오퍼레이터가 순서를 뒤집어 재개방된다. 따라서 **로마숫자를 실제 실행 순서로 정렬하고 각 스텝에 "선행 완료 실측 전 다음 금지"를 붙인다**:
   ...
   2. **protection-probe (ADR-0011 (4a)·목록 142.2)** — 일회용 **보호 브랜치**에 임시 규칙(count=0 + `require_code_owner_reviews=true` + "require a pull request before merging")을 걸어 **main 변경 전에** (i) code owner 승인 없는 머지가 실제로 차단되는지, (ii) PR 없는 direct-push가 거부되는지를 **거동으로 실측**한다(branch protection은 per-branch라 이 거동은 임시 브랜치에서 검증 가능). flip payload 문법·권한 리허설도 함께. **verdict-gate required *context*의 PR-head-SHA 바인딩만** main/PR-특정이라 post-flip에서 확증(point 7-e).  [ADR-0011 (4a)]
...
7. **최종 flip은 "완전-payload PUT → 즉시 검증 → 실패 시 원자 롤백"의 단일 트랜잭션이다.** 초안이 "단일 원자 PUT"의 원자성만 강조하고 (i) `PUT`의 전체-replace 시맨틱, (ii) code-owner 필드 보존, (iii) main-스코프 강제의 검증 위상을 누락한 것을 정정한다.
   ...
   - **(b) 완전 payload 재구성**: 스냅샷을 기반으로 오직 두 가지만 변경한 **완전한** 오브젝트를 만든다 — ① `required_approving_review_count` 1→0, ② `required_status_checks`에 **`verdict-gate`**(verdict-gate.yml이 PR head SHA에 게시하는 required check-run 이름, verdict-gate.yml:505-512) 추가. **🔴 payload는 반드시 `checks: [{context, app_id}]` 형태여야 하며 레거시 `contexts: [이름]`으로 재구성해서는 안 된다(codex #73 R7 [high])** — full-replace PUT에서 이름 리스트로 만들면 기존 **app_id 소스 핀이 조용히 강등**되고, `count=0` 이후 동명 required check가 비-sacred 경로의 주 게이트이므로 핀 상실은 곧 **check-위조 벡터**(ADR-0011 point 5 (b))의 재개방이다. 스냅샷의 기존 항목을 `context`+`app_id`째 그대로 복사하고 `verdict-gate`도 예상 소스 app_id와 함께 추가하며, **PUT 전 mandatory assertion**으로 "스냅샷이 app_id-핀인데 생성 payload에 app_id 없는 항목이 있으면 flip 중단"을 fail-closed로 건다. 이 변환은 prose가 아니라 **스크립트로 정본화하고 테스트**한다. 나머지는 **명시 보존**한다: `require_code_owner_reviews=true`(BLOCKING-1 — 이 필드가 사라지면 count=0 + code-owner off로 risk:critical PR이 verdict만 통과하면 사람 리뷰 없이 머지된다), `enforce_admins=true`, 기존 required contexts(`build · vet · gofmt · test-race`, app_id 고정 — ADR-0011 Context 실측), `restrictions`, `required_linear_history`, `required_pull_request_reviews` 블록 자체(제거 시 direct-push로 게이트 전체가 PR 경로째 우회 — ADR-0011 라운드 7).
   - **(c) PUT → 즉시 검증**: 단일 PUT 직후 곧바로 (1) `internal/enforcement`의 `CheckBranchProtection` 재통과 — **단 이 검사기는 `require_code_owner_reviews`만 파싱하고 `required_status_checks`는 보지 않으므로 code-owner 강제만 확인한다**(branchprotection.go:47-48), (2) **별도 `GET .../branches/main/protection` assertion으로 `required_status_checks`에 `verdict-gate` context가 실재하는지 명시 확인** — **verdict-gate required는 `CheckBranchProtection`이 아니라 이 GET assertion이 검증한다**(PUT payload가 verdict-gate를 누락·silent drop하면 code-owner만 통과하고 verdict 게이트 없이 count=0이 되는 false-green을 봉쇄 — codex #73 [high]/[P2]), (3) `GET` diff로 의도한 두 필드 외 **무변경 실측**(silent drop 검출), (4) **테스트 PR로 count=0에서 code-owner 리뷰와 verdict-gate가 실제로 blocking인지 거동 실측**한다. (주: `CheckBranchProtection`을 verdict-gate 필수까지 검증하도록 확장하면 **Phase A(verdict-gate 미-required) presence-check가 fail-closed로 깨지므로**, 그 확장은 boot presence-check가 아닌 flip-전용 함수여야 한다 — 이 ADR은 문서 assertion으로 처리하고 enforcement 확장은 후속 옵션(residual).)
   ...
   - **(e) code-owner 강제·direct-push 거부는 pre-main 임시 보호 브랜치에서 실측한다(ADR-0011 (4a) 준수 — hard precondition ④); post-flip main 테스트 PR은 재확증이다. verdict-gate required *context*의 PR-head-SHA 바인딩만 genuinely main/PR-특정이다(codex #73 R2 정정):** branch protection은 per-branch이므로 count=0 + `require_code_owner_reviews` 차단 거동과 PR-less direct-push 거부는 임시 보호 브랜치에서 **main 변경 전에** 거동으로 실측 가능하고, **ADR-0011 (4a)(사람 액션 (4a)·목록 142.2)가 이를 hard precondition ④로 요구한다.** 이 point의 초안("count=0 code-owner 강제는 main-스코프라 probe로 검증 불가, 최초 검증은 flip 직후")은 **ADR-0011 (4a)와 정면 모순**이었다 — config 표현성(어느 브랜치에나 count=0+code-owner 설정 가능)과 behavioral-scope(그 거동이 어느 브랜치에서나 강제됨)를 혼동한 과-교정이다. **genuinely main/PR-특정인 것은 `verdict-gate` required *context*의 바인딩뿐**이다 — verdict-gate.yml이 특정 PR head SHA에 check-run을 게시하므로(verdict-gate.yml:505-512), 실제 PR이 verdict green 전까지 머지 차단되는지는 flip 직후 테스트 PR로 확증한다. 순서: **④a probe(pre-main: code-owner·direct-push 거동 실측) → (4b) flip → post-flip 테스트 PR(재확증 + verdict-context 바인딩 확증)**, flip을 트랜잭션으로 감싸 verdict-context 검증 실패 시 원자 롤백.
...
8. **부트스트랩 완화는 최초 1회성 예외이지 상시 운영 경로가 아니며, 그 *자격*은 파일 존재가 아니라 App-작성 경로의 operational 여부로 판정한다.**
   ...
   - **완화하지 않는 것이 1순위 — 부트스트랩 머지는 per-merge admin bypass를 쓴다(codex #73 R4 [high] 대응, 초안 정정)**: 초안은 "protection을 일시 완화 → 머지 → 복원"을 부트스트랩 경로로 서술하고 복원 backstop을 "짧은 만료 또는 외부 알림 + 같은-세션 복원"으로 뒀는데, **알림은 롤백이 아니고** 구체적 메커니즘·타임아웃·소유자·복구 산출물이 없어 규율에 불과했다. pre-narrowing 구간은 정의상 loop가 admin/workflow/approve-capable `chnu-kim`을 쥔 적대 주체이므로, 완화 후 복원 전 세션·오퍼레이터 실패는 **서버-side에 durable한 무보호 main**을 남긴다. **정정: 부트스트랩 sacred PR은 `gh pr merge --admin`(per-merge bypass)으로 머지한다 — branch protection 설정을 전혀 바꾸지 않으므로(그 1건 머지에만 적용) 완화 창 자체가 생기지 않고 롤백할 상태도 없다.** 실증: 이 ADR을 포함한 이 세션의 부트스트랩 sacred PR 전부가 이 경로로 머지됐고 protection은 무변경으로 유지됐다. 전제는 `enforce_admins=false`(현 실측)와 머지 수행자의 admin 권한이다.
   - **완화 경로는 admin bypass가 불가능할 때(예: `enforce_admins=true`)만의 최후 수단**이며, 그 경우 다음이 **mandatory**다: (i) 완화 **전** `GET .../protection` **전체 스냅샷을 저장**해 복원 payload를 확보하고, (ii) branch-global 프리체크(위)를 수행하며, (iii) **구체적 데드라인 + 독립적으로 실행 가능한 복원 수단**(세션 실패 시 실행할, 두 번째 오퍼레이터가 보유한 복원 명령)을 완화 **개시 전에** 준비한다 — 알림만으로 갈음 금지, (iv) 복원 후 `CheckBranchProtection` + `GET` diff로 **원상태를 실측하고 그 증거를 기록**한다(복원도 destructive PUT이므로 self-assert 금지). 이 넷이 준비되지 않으면 완화를 개시하지 않는다(fail-closed).

### 이 ADR을 enforcement-integrity 우산에 편입 (twin-artifact)

9. 이 ADR은 게이트 강제 메커니즘의 활성화 절차·순서·flip 트랜잭션을 정의하므로, 미래의 loop가 이를 조용히 개정하면 enforcement가 약화된다. **같은 PR에서** (a) `.github/CODEOWNERS`에 `/docs/adr/0015-*.md @chnu-kim`을 추가하고, (b) presence-check check (a)의 기대 sacred 경로 집합(`internal/enforcement/codeowners.go`의 `sacredRequiredPaths`)에 이 파일의 실제 경로를 반영하며 (c) `internal/enforcement/codeowners_test.go`에 0015 누락 fail-closed 케이스를 0011/0012/0013 패턴대로 추가하고, (d) ADR-0011의 세 곳 — **point 1**(로컬 금지 원칙), **point 5**(Phase A/B precondition·② 완료 판정·flip), **Consequences 부트스트랩 절차 문단**(옛 "protection 일시 완화 → 복원"이 stale해진 곳; codex #73 R5가 이 누락을 잡았다 — 지배 ADR에 옛 절차가 남으면 stateless 오퍼레이터가 그걸 따라 main을 durable하게 무보호로 남길 수 있다) — 에 이 ADR로의 amend 포인터를 추가하고, (e) runbook `docs/runbooks/phase-b-entry.md`를 저작하고(#50), **(f) 그 runbook 자체도 (a)~(c)와 동일하게 sacred 등록한다** — `/docs/runbooks/phase-b-entry.md @chnu-kim` CODEOWNERS 규칙 + `sacredRequiredPaths` 등재 + 누락 fail-closed 테스트. **근거(codex adversarial #73)**: runbook은 활성화의 *실행* 단계(narrowing 순서·App key 프로비저닝·flip-and-verify-or-rollback·부트스트랩 프리체크)를 담은 **gate-defining 문서**이고 이 ADR이 그것을 지배 절차로 참조하므로, ADR만 보호하고 runbook을 무보호로 두면 loop가 App key 프로비저닝을 narrowing 앞으로 옮기거나 rollback을 빼는 식으로 **보호된 ADR을 우회해 활성화 경로를 조용히 약화**할 수 있다("main에 있음 ≠ 보호됨" — ADR-0011 point 4(b)). CODEOWNERS는 frontmatter를 읽지 못하므로 정적 경로 등록이 유일한 방법(ADR-0010 point 4)이고, 문서만 바꾸고 강제 쌍둥이를 남기면 게이트가 거짓 안심한다(MEMORY twin-artifact-coupling).
...
- **flip을 "단일 PUT"으로만 서술(검증 위상·code-owner 보존·replace 시맨틱 생략)** — 기각(렌즈-1 BLOCKING-1/2): `PUT`은 전체 replace라 빠진 필드가 리셋되고(특히 `require_code_owner_reviews`가 사라지면 sacred/live-execution 게이트 소멸). code-owner/direct-push 강제는 pre-main 임시 보호 브랜치에서 실측(ADR-0011 (4a)), verdict-context 바인딩만 post-flip. flip-and-verify-or-rollback 트랜잭션(point 7)으로 대체한다.
...
- (알려진 잔여 위험 — 중대) **count=0 code-owner 거동·PR-less direct-push 거부는 pre-main 임시 보호 브랜치에서 실측한다**(ADR-0011 (4a)·point 7-e — codex #73 R2 정정). flip 직후에야 권위 있게 검증 가능한 것은 `verdict-gate` required-context의 PR-head-SHA 바인딩뿐이다(verdict-gate.yml이 PR별 check-run 게시) — flip을 flip-and-verify-or-rollback 트랜잭션으로 감싸 이 검증 실패 시 즉시 스냅샷 롤백한다. 남는 노출 창(post-flip verdict-context 확증~롤백)은 트랜잭션 원자성으로 유계이며 0이 아니다.
...
- (제약 전파/부트스트랩) 이 ADR PR 자체가 부트스트랩 예외(App-작성 경로 미-operational sacred PR — point 8)다 — **protection을 완화하지 않고 `gh pr merge --admin`(per-merge bypass)으로 머지한다**(설정 무변경 → 완화 창·롤백 대상 없음). 완화 경로는 admin bypass 불가 시에만의 최후 수단이며 스냅샷·프리체크·독립 복원수단·복원 후 실측 증거 넷이 mandatory다(point 8). 이 예외는 이 ADR·최초 pr-creation.yml에 한정된다.
- (검증 방법) 활성화 완결의 검증은 fail-closed 순서로 성립한다: ② narrowing capability 실측(7-ix, loop 컨텍스트) → ①/⑤ presence-check·red-team leg별 → ④ probe(pre-main 보호 브랜치: count=0 code-owner 차단·PR-less direct-push 거부 **거동 실측** — ADR-0011 (4a)) → (1) rotate+environment → (4b) flip-and-verify-or-rollback 트랜잭션(즉시 `CheckBranchProtection`[code-owner만] + 별도 `GET` assertion[`verdict-gate` required 실재] + `GET` diff[무변경] + 테스트 PR[verdict-context 바인딩·code-owner 재확증] 실측, 실패 시 원자 롤백). 어느 단계 미통과면 Phase A에 남는다.
