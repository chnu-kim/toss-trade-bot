...
- (검증 방법) 활성화 완결의 검증은 fail-closed 순서로 성립한다: ② narrowing capability 실측(7-ix, loop 컨텍스트) → ①/⑤ presence-check·red-team leg별 → ④ probe(payload 리허설, main 무접촉) → (1) rotate+environment → (4b) flip-and-verify-or-rollback 트랜잭션(즉시 `CheckBranchProtection`[code-owner만] + 별도 `GET` assertion[`verdict-gate` required 실재] + `GET` diff[무변경] + 테스트 PR[count=0 code-owner/verdict blocking 거동] 실측, 실패 시 원자 롤백). 어느 단계 미통과면 Phase A에 남는다.
