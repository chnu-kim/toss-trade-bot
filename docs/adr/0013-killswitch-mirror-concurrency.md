...
verification:
  ...
  - reviewer: codex:github-bot (PR #63)
    date: 2026-07-17
    verdict: 6건 지적 → 전부 반영. bootHalt P2(수동 clear가 bootHalt 미clear→stuck-block)·bootHalt P1(clean-shutdown 자격이 bootHalt 누락→재시작 유실)·token P1(임계미달 persist 실패 non-reconstructable→래치 스코프를 decision-locus에서 '재구성 가능성'으로 정정)·enforcement P1(protects 선언에 CODEOWNERS/sacredRequiredPaths twin-artifact 배선 누락→등록)·W-C P1(idempotent-no-op 트립이 clear와 레이스→ClearHalt inflight-defer + delayed-halt booking, 과장 완화)·W-D P1(idempotent no-op이 evidence 발행까지 skip하면 실 트리거도 유실→evidence(카운터 tx·occurrence)는 halt 무관 항상 발행, halt write만 idempotent로 정정)·W-E P1(clear 후 임계-초과 evidence를 CanSubmit이 안 읽어 재-fire 지연→order-failure와 동종 bounded delayed-halt로 booking, #35 bounded LIVE re-count로 유계; 심층 봉쇄는 ADR-0012 소관 후속 이관 — 사용자 결정). 결정 불변, 배선·스코프·정직성 수정.
---
...
- **트립 유발 경로**(manual Trip global · ambiguous 에스컬레이션 · `ReportOrderFailure` · `ReportTokenRefreshFailure`):
  ...
  2. `haltMu` 획득.
  3. **트리거 evidence를 먼저 발행한다 — halt 상태와 무관(W-D, codex)**: count-first(`ReportOrderFailure`/`ReportTokenRefreshFailure`)는 카운터 read+증가를 자기 store tx로 durable commit하고(단일 writer가 임계 판정 직렬화 — ADR-0005), 임계 도달 시 **같은 tx에서** `TripHalt`(ADR-0012 count-first); ambiguous 에스컬레이션은 occurrence를 기록한다. **이미 `durableHalt==halted`여도 이 evidence 발행(카운터 tx·occurrence)은 건너뛰지 않는다** — 건너뛰면 W-C의 delayed-halt 근거(clear 뒤 재-fire할 persistent evidence)가 사라져 실 트리거도 bare `Trip`처럼 유실된다. **halt durable write만 idempotent**하다: 이미 `durableHalt==halted`면 halt 재작성을 skip(churn 방지, 미러 halted 유지), 아니면 `MarkHaltPending`→`TripHalt` commit 후 `durableHalt=halted`(I3). durable **에러**면 아래 "durable-error 래치는 '잃은 상태의 재구성 가능성'으로 스코프한다"대로 처리(재구성 불가 신호는 sticky 래치 set — count-first order-failure만 래치 없이 re-count로 복구).
  4. `haltMu` 해제.
...
- **`ClearHalt`**(수동, ADR-0004 point 6): `haltMu` 획득 → **`inflightTrips>0`이면 defer**(진행 중 트립이 있으니 clobber 금지 — 아래 W-C) → durable `ClearHalt` commit; 성공 시 `durableHalt=none` **및** 래치 clear **및 `bootHalt=false`**(operator clear가 in-memory 보수적/panic halt까지 실제로 풀어야 한다 — 안 그러면 durable 성공에도 `bootHalt`가 남아 영구 blocked, codex P2), 에러면 셋 다 유지(fail-closed) → 해제. **`inflightTrips`는 값을 바꾸지 않는다**(자기 owner만 dec) — 동시 트립의 `count>0`이 `durableHalt` 하강과 **독립적으로** block을 붙든다(carrier 발행 트립의 W4/W7 봉쇄).
  - **W-C(codex, idempotent-no-op 트립 vs clear)**: 이미 `durableHalt==halted`일 때 fresh 트립이 idempotent no-op 경로(자기 carrier 미발행)를 타면, 동시 clear가 halt를 내린 뒤 그 트립이 dec하면 재개방된다. **defer 규칙**(clear는 `inflightTrips>0`인 동안 진행 안 함)이 "트립이 clear 임계구역과 겹치는" 순서를 닫는다. "트립이 완전히 끝난 뒤 clear" 순서는 trigger의 **persistent evidence**(order/token durable 카운터 · ambiguous journal)가 재-fire하는 **delayed-halt**로 흡수된다(order-failure와 동일 계약) — 이 흡수는 **step 3 W-D**(evidence 발행은 halt 여부와 무관하게 항상 수행)에 의존한다. evidence 발행을 idempotent no-op이 건너뛰면 실 트리거도 재-fire 근거를 잃어 이 흡수가 무효가 된다. 유일한 완전-유실 잔여는 **evidence 없는 bare one-shot `Trip(global)`이 halt 중 발생 + clear와 레이스**하는 operational edge(operator가 trip과 clear를 동시 지시)뿐이다.
  - **W-E(codex, clear vs 임계-초과 evidence — bounded delayed-halt)**: 실 트리거가 halt 중 임계를 넘어(W-D로 evidence는 durable) 그 뒤 operator가 clear하면, `CanSubmit`은 **evidence store(카운터·occurrence)를 읽지 않으므로** clear 직후 재개방된다 — delayed-halt의 재-fire는 "즉시"가 아니라 **다음 reporter/reconciler 이벤트에서** 일어난다. 이 window는 **reconciler re-count cadence로 유계(bounded)**이며, order-failure durable-error의 delayed-halt와 **동일한 계약**이다(둘 다 "evidence는 durable, 재-fire는 reconciler가 완성"). 따라서 이 window를 유계로 만들려면 **#35 reconciler가 미해결 order intent·임계-초과 카운터를 bounded LIVE cadence로 재평가**해야 한다(정적 상황에서도 유계 — Consequences의 order-failure delayed-halt booking과 같은 요구를 escalation 카운터 전반으로 확장). **`CanSubmit`을 evidence store 읽기로 만들지 않는다**(ADR-0004 point 5 핫패스 값싸게 — evidence는 durable store라 round-trip). 이 window의 더 강한 봉쇄(이유별 halt · clear가 카운터 리셋 · evidence-as-in-process-carrier)는 **ADR-0012 escalation-카운터 모델과 얽힌 별개 설계 포크**라 이 ADR 범위 밖 후속으로 이관한다(필요 시 별도 `/architect`). ADR-0013은 미러-정합성 모델을 확정하고, 이 clear-vs-evidence window를 order-failure와 동종의 bounded delayed-halt로 정직히 booking한다.

  count-first·ambiguous·token 등 실 트리거는 전부 위 bounded delayed-halt로 재-fire한다(아래 Consequences booking).
...
- (비용/한계) **clear 경로의 두 delayed-halt window를 정직히 booking한다 — "모든 clear-vs-trip이 구조적으로 닫힌다"는 과장이다.** (1) **W-C(idempotent-no-op 트립 vs clear)**: 이미 durably halted일 때 fresh 트립은 carrier를 안 만들어, `ClearHalt`의 `inflightTrips>0` defer가 겹치는 순서를 닫고 실 트리거의 evidence 재-fire가 나머지를 흡수한다(bare one-shot `Trip(global)`의 operational edge만 완전-유실 잔여). (2) **W-E(clear vs 임계-초과 evidence)**: 실 트리거가 halt 중 임계를 넘고 clear되면, `CanSubmit`이 evidence store를 안 읽어 재-fire가 다음 reconciler 이벤트로 미뤄진다 — order-failure delayed-halt와 **동종의 bounded window**다. 두 window 모두 **#35 reconciler의 bounded LIVE re-count cadence**가 유계로 만든다(정적 상황에서도 유계 — #35 AC). 더 강한 봉쇄(이유별 halt·clear-리셋·evidence-as-carrier)는 ADR-0012 escalation 모델과 얽힌 별개 포크라 후속 이관.
