...
7. **precondition ②의 두 순환 의존을 다음과 같이 해소한다 — `verify-credential-narrowing.sh`를 수정한다.**
   - **(a) APPROVE 프로브 대상**: `APPROVE_TARGET_PR`은 보조 계정(`chanwoo040531`)의 **fork에서 연 draft PR**로 하고, base를 step 2의 probe 보호 브랜치로 둔다. fork PR은 write 권한이 필요 없고, PR 자체는 base 레포에 존재하므로 `POST /pulls/{n}/reviews`의 정당한 대상이 된다. draft + probe-base이므로 승인이 성공(=narrowing 실패)해도 main으로 아무것도 흐르지 않는다. 실측 직후 close한다.
