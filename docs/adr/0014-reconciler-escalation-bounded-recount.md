...
- (좋음) persistence-wins가 **no-guard로 흡수**된다(Decision 8) — reconciler는 prune/resolve 게이팅에 어떤 halt carrier도 읽지 않고, D1(c) hazard는 ambiguous(acked/orderId 부재→구조적 unresolvable)·order-failure(count-before-resolve, prunable↔non-durable 상호배타)로 unreachable하다. 덕분에 사람-수동-clear 전용 전역 halt·`bootHalt`가 장기 지속돼도 confirmed 주문 resolve·감사 finalize가 **동결되지 않는다**(codex R2/R3 회귀 봉쇄; `HasUnpersistedPendingHalt`는 bootHalt에도 true라 loss-window 신호로 부적합). 향후 ambiguous에 resolvable 핸들이 생기면 `bootHalt`·durable `pending`/`halted`를 **제외**한 loss-window 전용 가드로만 복원한다(twin-artifact 복원 조건).
