...
verification:
  ...
  - reviewer: codex:adversarial-review
    date: 2026-07-18
    verdict: 2 rounds — R1 needs-attention(high, No-ship): Accepted ADR-0004 amend가 Proposed ADR-0014에 의존하는 split-brain → ADR-0014를 같은 ship-ready PR에서 Accepted로 flip(ADR-0009 point 1 위임 승인). R2 needs-attention(high, No-ship): Decision 8 persistence-wins가 durable `halted` 전체에 적용돼 수동-clear-전용 전역 halt 지속 중 confirmed 주문 resolve·감사 finalize가 동결(availability 자해) → 가드를 loss window(unpersisted/durable-pending)로 좁히고 durable `halted`에선 finalization 계속(신규 제출은 CanSubmit이 차단하므로 안전 불변). 두 라운드 모두 새 결함, 진동 아님.
  - reviewer: chnu-kim
    date: 2026-07-18
...
8. **순서 계약(어긴 순간 fail-open):**
   ...
   - **persistence-wins(ADR-0012 D1(c)) — loss window(아직 durable하지 않은 halt)에만 적용, durable `halted`에선 finalization을 계속한다(Lens1 F3 봉쇄 + codex adversarial R2 [high] 반영):** reconciler가 **evidence를 제거하는 행위**(acked→terminal `ResolveIntent`, prepared→aborted `ResolveIntent`, `FinalizeFullyAudited`/감사 finalize로 not-fully-audited 레코드 pruning) **직전에**, halt가 **아직 durable하지 않은 loss window**인지 확인한다 — 즉 `killswitch.HasUnpersistedPendingHalt()`가 true(in-memory-only pending: `MarkHaltPending` 실패 arm·token latch·panic 승격 bootHalt)이거나 durable `store.Halt().Phase == pending`(durable 커밋 진행 중)이면 그 pruning을 **보류**한다. durable `store.Halt()`(미러 아님)와 `HasUnpersistedPendingHalt()`를 **둘 다** 읽어야 하는 이유는, durable만 읽으면 **durable=none인데 in-memory로만 halt**인 상태를 `HaltNone`으로 오독해 loss window의 절반을 놓치기 때문이다.
     - **그러나 durable `store.Halt().Phase == halted`(완전 커밋)이면 pruning을 보류하지 않고 finalization·prune를 계속한다.** 그 halt는 재시작에 durable하게 생존하므로 evidence pruning이 halt 재구성을 깨지 않는다(reconciler가 pruning하는 acked-order evidence는 애초에 **어떤 halt도 재구성하지 않는다** — order-failure count는 count-before-resolve(Decision 8)로 이미 durable, ambiguous backlog evidence는 reconciler가 건드리지 않는다(Decision 4)). 가드를 `halted` 전체로 넓히면 **전역 halt가 사람 수동 clear 전용(Decision 5)이라 며칠 durable하게 서 있는 동안 확정된 주문 truth·감사 수렴이 전부 동결**돼, 인시던트 중(=halt 켜진 때)에 복구 상태가 오히려 stale해지는 availability 자해가 된다(codex R2 [high]). 신규 노출 제출은 그 창 내내 `CanSubmit`이 계속 차단하므로 finalization 계속이 안전을 약화하지 않는다 — 동결되는 것은 비가역 노출이 아니라 bookkeeping이다.
     - (초안이 이 가드를 "unresolved-ambiguous resolve 전"에 걸었던 것은 **오배치** — reconciler는 ambiguous를 resolve하지 않는다(Decision 4). 올바른 대상은 reconciler가 실제로 밟는 evidence-pruning 경로다.)
...
9. **부팅 스캔 = 한 부팅 goroutine 안 두 순차 패스. 패스 1은 분류+Trip 주입만 하고 gate를 연다 — open 주문 추적은 gate를 막지 않는다(Lens1 F4 봉쇄).**
   ...
   - **패스 2 — 감사 re-emit 드라이버(원천: `store.LoadNotFullyAuditedIntents`, `ReconstructLifecycleRecords` audit_ack.go:53-86):** 미-ack lifecycle 레코드를 결정적으로 재도출해 sink로 re-emit(멱등키 병합) + `RecordAuditAck`/`FinalizeFullyAudited`로 플래그 수렴(#20 잔여, ADR-0006 point 4). **이 패스는 gate-open 뒤에 돌아도 안전**하다 — 감사 re-emit은 신규-노출 차단을 만들지 않는다(intent는 이미 resolve됨). 유일한 차단 효과인 감사 emit 실패(`FailClosedError`)→`Trip(ScopeGlobal)`(ADR-0006 point 6)는 durable fail-closed라 gate-open 뒤에 걸려도 재-block한다. **단 `FinalizeFullyAudited`는 Decision 8 persistence-wins 가드를 통과할 때만 실행한다**(pending/unpersisted halt 창이면 finalize 보류; durable `halted`이면 계속 — Decision 8).
...
- **persistence-wins 가드를 durable `halted` 상태 전체에 적용** — 기각(codex adversarial R2 [high]): 전역 halt가 수동-clear 전용(Decision 5)이라 며칠 durable하게 서 있을 수 있는데, 그 내내 확정된 주문 `ResolveIntent`·감사 finalize가 동결되면 인시던트 중 복구 상태가 stale해지고 감사 ack/prune가 수렴하지 못한다(availability 자해). evidence 소멸 위험은 **halt가 durable하지 않을 때만** 존재하므로 가드를 loss window(`HasUnpersistedPendingHalt()` true 또는 durable `Phase == pending`)로 좁히고, durable `halted`에선 finalization을 계속한다 — 신규 노출 제출은 `CanSubmit`이 계속 차단하므로 안전 불변.
...
- (제약 전파) **#35 AC를 확장해야 한다**(현 AC에 없음): (a) supervised LIVE re-count cadence + **조용한 시장 재-fire·종목 floor 전이** 유계 테스트(시계 주입 ticker), (b) 두 패스 부팅 순서 테스트(모든 Trip이 `NotifyScanComplete` 전 주입; open 주문은 gate를 막지 않음), (c) `acked` GetOrder 유계 소진이 gate를 안 막고 unresolved 재-drive로 남는 테스트, (d) **성공-리셋 순서 가드 테스트**(더 이른 in-doubt REJECT가 나중 FILL 리셋에 삼켜지지 않고 임계 도달 시 트립), (e) **ClearSymbol 잔여-0 테스트**(한 종목 다중 ambiguous 중 하나만 해소 시 종목 차단 유지), (f) **persistence-wins loss-window 한정 테스트**: (f1) durable=none·in-memory-only pending halt(`HasUnpersistedPendingHalt` true)에서 evidence pruning **보류**, (f2) durable `Phase==pending`에서 보류, (f3) **durable `halted`(완전 커밋)에서는 confirmed 주문 `ResolveIntent`·감사 finalize가 보류되지 않고 계속됨**(codex R2 [high] — halt 지속 중 finalization 동결 방지), (g) **reconciler 설정 zero-threshold fail-closed 거부 테스트**, (h) **ticker 사망 fail-closed 승격 테스트**. 기존 AC(마커 3분기·ABSENT/auto-ack 금지·count-before-resolve·settle window)는 유지.
