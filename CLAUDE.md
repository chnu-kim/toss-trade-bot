...
- App installation access token 발급 로직은 `internal/enforcement.InstallationTokenMinter`(`internal/enforcement/installtoken.go`)에 있다. **이 경로는 App 자격증명(App ID·installation ID·private key)이 실제로 공급됐을 때만 쓴다** — 공급되지 않으면(현재 기본 상태) `.claude/skills/dispatch-issue/`가 기존 `gh auth switch --user <사람 계정>` 흐름으로 폴백한다. 구체적 CLI 사용법(토큰을 git/gh에 먹이는 법)은 `.claude/agents/go-tdd-implementer.md`.
- 이 전환은 설계 단계다 — 실제 `mechanu[bot]` identity로 만든 커밋/PR이 GitHub에서 그렇게 표시됨을 실측 확인한 적은 아직 없다(#43. 검증은 별도 진행). **알려진 잔여 위험**(codex adversarial-review 지적): App 자격증명이 배선되기 전까지는 이 폴백 때문에 `#43`이 풀려던 self-approval 교착이 그대로 남는다 — 사람 게이트(PR 검수·머지) 자체는 우회하지 않지만, 다른 협업자 계정 승인이나 사람의 직접 병합이 여전히 필요하다. 이 폴백을 hard stop으로 바꿀지는 `/dispatch-issue` 가용성 전체에 영향을 주는 별도 정책 결정이라 이 문서 갱신 범위 밖이다.
