...
> **회귀 검사**: `grep -n -- '--wait --base main' .claude/agents/go-tdd-implementer.md`가
> **아무것도 반환하지 않아야** 한다(명령 형태를 정확히 겨냥한다 — 이 각주처럼 규칙을 *설명하는*
> 산문에는 그 문자열이 들어가므로, `--base main`만으로 grep하면 자기 문서를 잡는 거짓 양성이 난다).
> `gh pr create --base main`은 PR의 *base 브랜치*라 완전히 별개이므로 그대로 둔다.
>
> *이 각주 자체가 회귀의 산물이다*: 불변식을 산문으로만 적고 흐름 끝의 실제 명령을 안 고쳤다가
> 적대 리뷰가 잡았다(PR #81). 그리고 첫 수정에서 쓴 회귀 검사마저 자기 문서를 잡는 부정확한
> 형태였다. **규칙 · 그 규칙이 지배하는 명령 · 그 규칙을 지키는 검사** 셋이 같은 변경에서
> 함께 움직여야 한다.
