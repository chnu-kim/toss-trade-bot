...
그 호출 자체가 타임아웃/에러로 끝나면(스스로 "5분 무진전"을 판단하지 마라 — 느리지만 정상 진행 중인 리뷰와
진짜 멈춤을 로그만으로 구분할 수 없다) **한 번 재시도**하고, 재시도도 실패하면 그때만 취소하고 셀프 적대적
재검토로 대체한다 — 이 대체는 **반드시 최종 반환의 "판단"과 "마찰"에 명시**해 오케스트레이터·사용자가 필수
독립 리뷰가 생략됐음을 알 수 있게 한다(조용히 넘어가지 않는다).
