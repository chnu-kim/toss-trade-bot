...
7. **무언가를 게이트·강제·증거 생성기로 승격하면, 같은 PR에서 그것을 sacred로 등재한다**(CLAUDE.md 규칙).
   ...
   누락 fail-closed 테스트.
8. **push·서명이 막히면 원인을 *먼저 판별*한 뒤에만, 그리고 한 가지 원인에 한해 우회한다.**
   - **우회 가능(로컬 환경 문제)**: 시크릿 매니저/ssh-agent가 잠겨 키를 아예 못 주는 경우 —
     `ssh-add -l`이 "no identities"이고 서명·push가 함께 실패한다. 이때는 `required_signatures`가
     false인지 확인 후 `git -c commit.gpgsign=false commit`, push는 gh 자격증명 기반 HTTPS URL.
     **우회 사실을 보고에 남긴다**(커밋이 Unverified로 남는다).
   - **🔴 우회 금지(자격증명 상태 문제)**: `origin`이 여전히 SSH인데 push가 **거부**되는 경우는
     narrowing 후 signing-only key로 authentication이 거부되는 **의도된 동작**일 수 있다(ADR-0011
     실측 목록 8, 아래 "완료 절차" 각주). 이 경우 **HTTPS로 갈아타서 통과시키지 마라** — 그건
     미완의 narrowing/remote 전환을 감추고 모호한 자격증명 상태로 흐름을 계속시킨다. **멈추고**
     #46 완료 여부와 `git remote -v`의 HTTPS 전환을 오케스트레이터에게 확인한다.
   - 판별 기준: **키 자체를 못 얻는가**(로컬 잠김 → 우회 가능) vs **키는 있는데 서버가 거부하는가**
     (narrowing 상태 → 정지·보고). 구분이 안 되면 **정지·보고**가 기본값이다.
...
> 🔴 **codex 리뷰의 base는 `origin/main`이다**(운영 불변식 1). 워크트리의 로컬 `main` ref는 갱신되지
> 않아 stale diff로 이미 머지된 남의 변경을 리뷰 대상에 포함시킨다. 편집 시 되살리지 마라.
>
> **회귀 검사**(이 파일에서 실행, 무출력이어야 함):
> ```bash
> grep -n 'codex-pr-review' .claude/agents/go-tdd-implementer.md | grep -v 'origin/main'
> ```
> codex 호출을 언급하는 줄 중 `origin/main`이 없는 줄을 찾는다. **이 검사는 금지 리터럴을 스스로
> 적지 않는다** — 금지 문자열을 문서에 쓰면 그 문서가 자기 검사에 걸려 영구 red가 되고, 워커가
> 그 red를 무시하도록 훈련된다. (`gh pr create`의 base 지정은 PR의 *base 브랜치*라 완전히 별개이며
> 이 검사에 걸리지 않는다.)
>
> *이 각주는 회귀의 산물이다*: 불변식을 산문으로만 적고 흐름 끝의 실제 명령을 안 고쳤다가 적대
> 리뷰가 잡았고(PR #81 [high]), 그 수정에서 쓴 **회귀 검사마저 자기 문서를 잡아 영구 red**였다(같은
> PR [medium]). **규칙 · 그 규칙이 지배하는 명령 · 그 규칙을 지키는 검사** 셋이 같은 변경에서
> 함께 움직여야 하고, **검사는 실제로 실행해 green을 확인**해야 한다(작성만으로는 검사가 아니다).
