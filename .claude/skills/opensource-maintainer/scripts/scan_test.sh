...
echo "== M-6b: 일반 코드 식별자 대입은 과검출 안 함 (codex P2) =="
D=$(make_repo \
  'field.go|clientSecret: clientSecret,' \
  'fake.go|GH_TOKEN=fake-token-never-used')
OUT=$(run_scan bash "$D" --all); RC=$?
{ ! grep -q 'field.go' <<<"$OUT"; } && pass "구조체 필드 clientSecret: clientSecret 미검출" || fail "식별자 대입 과검출(field.go)"
{ ! grep -q 'fake.go'  <<<"$OUT"; } && pass "따옴표없는 숫자없는 값 미검출(fake-token)" || fail "숫자없는 값 과검출(fake.go)"
[ "$RC" -eq 0 ] && pass "식별자만 있는 레포 exit 0(거짓 양성 없음)" || { fail "식별자 대입인데 exit=$RC"; printf '%s\n' "$OUT"; }
rm -rf "$D"
