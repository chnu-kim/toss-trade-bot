...
# 제외 목록(placeholder·example 등)은 **매치된 부분(키워드+구분자+값)** 에만 적용한다.
# 줄 전체에 적용하면 같은 줄 아무 데나 그 단어를 적어 진짜 자격증명을 숨길 수 있다
# (`client_secret: <실값>  # example value copied from docs`). CI가 PR 내용을 이 스캐너로
# 검사하므로, 그런 인라인 우회는 스캔 대상과 같은 신뢰 경계 안에 있다 = false-green.
# 값 밖의 주석·산문은 이제 제외 판정에 영향을 주지 않는다. 의도적 허용은 CODEOWNERS로
# 보호되는 allowlist 매니페스트로만 한다.
exclude_by_matched_value() { # $1=탐지 패턴 $2=제외 regex $3=git grep 플래그
  local pat="$1" excl="$2" flags="$3" line content matched gi=""
  if [ -z "$excl" ]; then cat; return; fi
  case "$flags" in *-i*) gi="-i" ;; esac
  while IFS= read -r line; do
    [ -z "$line" ] && continue
    content="${line#*:}"; content="${content#*:}"
    matched=$(printf '%s\n' "$content" | grep -oE $gi "$pat" | head -1)
    if printf '%s' "$matched" | grep -qE $gi "$excl"; then continue; fi
    printf '%s\n' "$line"
  done
}
...
scan() { # $1=설명 $2=패턴 $3(opt)=제외 grep -vE 패턴 $4(opt)=git grep 플래그(예: -i) $5(opt)=추가 제외 pathspec
  ...
  out=$(printf '%s\n' "$out" | exclude_by_matched_value "$2" "${3:-}" "$extra")
  out=$(printf '%s\n' "$out" | apply_allowlist)
  out=$(printf '%s\n' "$out" | grep -v '^$')
