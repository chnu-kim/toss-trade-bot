...
**고-stakes ADR은 프리뷰 전에 하드닝한다.** 비가역·자금·인증·무인 안전을 지배하는 ADR은, 사용자에게 승인받기 *전에* adversarial 검토를 **수렴(approve)까지 반복**한다 — 한 번이 아니다(실측: 깊은 결함은 라운드를 거듭할수록 드러난다 — 스펙 사실 → 내부 정합성 → 물리 내구성). 메커니즘: `advisor`(PR·커밋 불필요 — 지금 바로 돌린다) 또는 `codex adversarial-review`(`codex-pr-review` 스킬/companion). **codex 경로 순서**: codex는 diff를 봐야 하므로 Proposed ADR을 **피처 브랜치에 먼저 커밋**(여전히 `Proposed`) → codex를 **수렴(approve)까지** → 결함 반영 → 같은 브랜치에서 `Accepted`로 flip → PR. 즉 이 경로에선 §4의 커밋을 승인 앞으로 당기되(브랜치의 Proposed 커밋은 main이 아니라 무해), **`Accepted` 전환은 여전히 하드닝·사용자 승인 뒤**다. codex가 advisor가 놓친 결함을 잡은 전례가 있으니(ADR-0006) 고-stakes엔 codex 경로를 우선한다. **이유**: `Accepted`는 issue-drafter·dispatch-issue가 링크·구현하는 신호라, 하드닝 전 버전이 Accepted가 되면 결함 든 결정이 소비될 수 있다. 하드닝은 라운드를 늘리는 게 아니라 **PR 뒤에서 앞으로 앞당기는** 것 — 사용자는 이미 굳은 결정을 승인한다. (근거: 회고가 ADR-0002·0006에서 이 패턴을 잡았다.)
...
- **ADR이 `protects:`를 비어있지 않게 선언하면 twin-artifact를 같은 PR에서 배선한다** — 그 ADR 경로를 `sacredRequiredPaths`(`internal/enforcement/codeowners.go`) + `.github/CODEOWNERS`에 등재(검증 fixture 포함). 이 완결성(선언↔배선)은 `internal/enforcement`의 `protects`-completeness 테스트가 CI에서 fail-closed로 강제한다(이슈 #64; 그 테스트가 서기 전까진 이 배선을 수동 확인). presence-check만 믿지 말 것 — twin-artifact 미배선은 memory·주석·prose 체크가 반복 non-firing한 재발 패턴이다([[twin-artifact-coupling]]).
