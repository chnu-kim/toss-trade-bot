...
> ⚠️ **App 토큰 hand-off는 아직 배선되지 않았다(codex GitHub-native review 지적, PR #44)**: 위 변수
> 블록엔 App 자격증명/토큰 관련 항목이 **없다** — `go-tdd-implementer.md`의 "완료 절차"가 전제하는
> `GIT_APP_TOKEN`을 이 §3이 실제로 채워 넘기는 경로는 이 시점 기준 **정의되지 않았다**. 즉 오늘은
> App 자격증명이 이 세션에 있어도 서브에이전트에 전달할 방법이 없어 **항상** §0의 사람 계정 폴백
> 경로로 간다. 이 hand-off(오케스트레이터가 직접 토큰을 발급해 변수로 넘길지, App 자격증명 자체를
> 넘기고 서브에이전트가 발급하게 할지 등)를 어떻게 설계할지는 별도 정책 결정이라 이번 문서 갱신
> 범위 밖으로 남겨둔다 — 실제로 필요해지면 이 변수 블록에 항목을 추가하고 이 경고를 지운다.
